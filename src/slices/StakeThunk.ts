...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const ohmContract = new ethers.Contract(addresses[networkID].LDAO_ADDRESS as string, ierc20Abi, signer);
    const sohmContract = new ethers.Contract(addresses[networkID].SLDAO_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
